...
int main(void) {
    ...
    while ((lineLength = getline(&line, &linecap, infile)) != -1) {
        ...
        SHA512((unsigned char*)line, lineLength, sha512hash);

        // Print hash as hex string
        char hexBuffer[2 * SHA512_DIGEST_LENGTH + 1];
        fputs(hexString(sha512hash, SHA512_DIGEST_LENGTH, hexBuffer), outfile);
        fputc('\n', outfile);
    }
    free(line);
