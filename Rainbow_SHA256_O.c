...
int main(void) {
    ...
    while ((lineLength = getline(&line, &linecap, infile)) != -1) {
        ...
        SHA256((unsigned char*)line, lineLength, sha256hash);

        // Print hash as hex string
        char hexBuffer[2 * SHA256_DIGEST_LENGTH + 1];
        fputs(hexString(sha256hash, SHA256_DIGEST_LENGTH, hexBuffer), outfile);
        fputc('\n', outfile);
    }
    free(line);
